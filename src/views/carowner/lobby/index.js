...
class Lobby extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      addressHere: [],
      tabIndex: getParamsObj().tabsIndex ? +getParamsObj().tabsIndex : 0,
      gotime: '',
      carCodeList: [],
      mobileChanged: false
    }
  ...
  getTabIndex() {
    ...
    let params = getParamsObj();
    const tabIndex = params.tabsIndex ? +params.tabsIndex : 0
  ...
  setIndex(v, i, cid = '', million) {
    window.scrollTo(0, 0)
    localStorage.setItem('tabsIndex', i)
    this.setState({
      tabIndex: i
    })
    if (i === 1 || i === 2 || i === 3) {
      ...
      if (!accessToken) {
        ...
        if (cid || million) {
          if (url.indexOf('?') > -1) {
            url += `&modelType=${cid}&drivingMileage=${million}&tabsIndex=${i}`
          } else {
            url += `?modelType=${cid}&drivingMileage=${million}&tabsIndex=${i}`
          }
        }
